			...
			if ($user) { // FB OBJECT "USER" OK
				try { 
					...
					if(mysql_num_rows($res)>0) { // USER EXIST IN THE DATA BASE
						$sql = 'UPDATE loreal_users SET redken_code="" WHERE fbid="'.$user_profile["id"].'"';
						mysql_query($sql);					
					}		
			...
			if ($user) { // FB OBJECT "USER" OK
				try { 
					...
					if(mysql_num_rows($res)>0) { // USER EXIST IN THE DATA BASE
						...
						$sql = 'UPDATE loreal_users SET redken_code="'.$redken_code.'" WHERE fbid="'.$user_profile["id"].'"';
						mysql_query($sql);						
						...
						$sql = 'INSERT INTO loreal_users (id, fbid, name, surname, date_birth, email, address, city, postcode, primary_reason, news, supercuts_salon_1, supercuts_salon_2, supercuts_salon_3, redken_code, supercut_code, created, modified) VALUES (null,"'.$user_profile["id"].'", "", "", "", "", "", "", "", "", "0", "", "", "", "'.$redken_code.'", "", "'.date("Y-m-d H:i:s").'", CURRENT_TIMESTAMP())';
						mysql_query($sql);
